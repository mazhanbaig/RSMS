...
import { Home, Bed, Bath, Square, MapPin, Calendar, DollarSign, ChevronRight, Heart, Users, Sparkles } from "lucide-react";
import { useRouter } from "next/navigation";
import { useCallback, useMemo } from "react";
import Image from "next/image";
        ...
        >
            ...
            <div className="my-auto">
                <Image
                    ...
                    width={200}
                    height={100}
                    className="object-cover w-full sm:w-100 max-h-60 rounded-lg"
                />
            </div>
            ...
            <div className="flex-1 px-2 py-3 sm:p-4 md:p-5">
                <div className="flex flex-col sm:flex-row 
                    ...
                    <div className="w-full sm:w-auto">
                        <h3 className="font-bold text-sm sm:text-base md:text-lg 
                                       mb-0.5 sm:mb-1 group-hover:text-indigo-600 
                                       transition-colors line-clamp-1
                ...
                <div className="flex flex-wrap items-center gap-3 sm:gap-4 md:gap-6 
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                        <Square className="h-3 w-3 sm:h-4 sm:w-4 text-gray-400" />
                        <span className="text-xs sm:text-sm text-gray-600">
                            {property.area || property.squareFeet || '—'} sqft
                        </span>
                    </div>
                </div>
                ...
                {/* Status Badge */}
                {property.status && (
                    <div className={`inline-flex px-2 py-0.5 rounded-md ${getStatusColor(property.status)} mb-2`}>
                        <span className="text-[10px] font-medium uppercase">{property.status}</span>
                    </div>
                )}
                ...
                <div className="flex flex-col xs:flex-row items-start xs:items-center 
                    ...
                    <button className="w-full xs:w-auto px-3 sm:px-4 py-1.5 sm:py-2 
                                         ...
                                         rounded-lg transition-colors 
                                         text-xs sm:text-sm font-medium
                                         border border-indigo-200 xs:border-0">
                        View Details →
                    </button>
                </div>
            </div>
