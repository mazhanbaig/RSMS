...
export const FeaturesGrid = () => {
    ...
    return (
        ...
        >
            ...
            >
                <div className="absolute top-40 right-10 w-80 h-80 bg-indigo-200/20 rounded-full blur-3xl" />
                <div className="absolute bottom-20 left-10 w-96 h-96 bg-purple-200/20 rounded-full blur-3xl" />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-gradient-to-r from-indigo-100/5 via-purple-100/5 to-pink-100/5 rounded-full blur-3xl" />
            ...
            <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-20">
                    {features.map((feature, idx) => (
                        ...
                        >
                            <div className={`relative bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-300 h-full overflow-hidden`}>
                                {/* Gradient Border on Hover */}
                                <div className={`absolute inset-0 bg-gradient-to-r ${feature.gradient} rounded-2xl opacity-0 group-hover:opacity-10 transition-opacity duration-500`} />

                                <div className="relative p-6">
                                    {/* Icon with Animation */}
                                    <motion.div
                                        animate={{
                                            rotate: expandedIndex === idx ? [0, 360] : 0,
                                            scale: expandedIndex === idx ? [1, 1.1, 1] : 1
                                        }}
                                        transition={{ duration: 0.5 }}
                                    ...
                                    >
                                        <div className="flex items-center justify-between">
                                            ...
                                            <motion.div
                                                animate={{ x: expandedIndex === idx ? 5 : 0 }}
                                    ...
                                    <AnimatePresence>
                                        {expandedIndex === idx && (
                                            ...
                                            >
                                                ...
                                                <div className="text-xs text-slate-400 mb-3">{feature.statLabel}</div>
                ...
                {/* Integration Trust Badge */}
                <motion.div
                    initial={{ opacity: 0 }}
                    whileInView={{ opacity: 1 }}
                    transition={{ duration: 0.6, delay: 0.8 }}
                    viewport={{ once: true }}
                    className="flex flex-wrap items-center justify-center gap-6 pt-6 border-t border-slate-200"
                >
                    <div className="flex items-center gap-2">
                        <div className="flex -space-x-2">
                            {[1, 2, 3, 4].map((i) => (
                                <div key={i} className="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-400 to-purple-400 border-2 border-white shadow-sm" />
                            ))}
                        </div>
                        <span className="text-sm text-slate-500">Integrated with</span>
                    </div>
