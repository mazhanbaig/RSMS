...
import {
    Menu, X, LayoutDashboard, Building2, Users, UserCircle,
    Calendar, LogOut, Settings, ChevronDown, Bell, Search,
    Plus, List, UserPlus, UserCheck, CalendarPlus, CalendarDays,
    Home, Briefcase, FileText, Mail, Phone,
    Apple
} from "lucide-react";
import Link from "next/link";
import { useState, useEffect, useRef } from "react";
...
export default function Header({ userData }: { userData?: Partial<UserInfo> | null }) {
    ...
    // Dropdown items for each tab
    const dropdownItems = {
        Properties: [
            { label: "All Properties", href: `/realstate/${userData?.uid}/properties`, icon: List },
            { label: "Add Property", href: `/realstate/${userData?.uid}/properties/add`, icon: Plus },
        ],
        Clients: [
            { label: "All Clients", href: `/realstate/${userData?.uid}/clients`, icon: Users },
            { label: "Add Client", href: `/realstate/${userData?.uid}/clients/add`, icon: UserPlus },
        ],
        Owners: [
            { label: "All Owners", href: `/realstate/${userData?.uid}/owners`, icon: UserCircle },
            { label: "Add Owner", href: `/realstate/${userData?.uid}/owners/add`, icon: Plus },
        ],
        Events: [
            { label: "All Events", href: `/realstate/${userData?.uid}/events`, icon: Calendar },
            { label: "Add Event", href: `/realstate/${userData?.uid}/events/add`, icon: CalendarPlus },
        ],
    };
  ...
  `}
            >
                <nav className="flex items-center justify-between">     {/* Logo Section */}
                    ...
                    <div className="hidden md:flex items-center bg-white/60 backdrop-blur-md rounded-2xl p-1 border border-slate-200/50 shadow-inner">
                        {navLinks.map((link) => {
                            const Icon = link.icon;
                            const active = pathname.startsWith(link.href);
                            const hasDropdown = ["Properties", "Clients", "Owners", "Events"].includes(link.label);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {hasDropdown && (
                                            <ChevronDown size={14} className={`relative z-10 transition-transform duration-200 ${isHovered ? 'rotate-180' : ''}`} />
                                        )}
                                    </Link>
                                    ...
                                    {hasDropdown && isHovered && (
                                        <div
                                            className="absolute left-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 animate-in fade-in slide-in-from-top-2 duration-200 z-50"
                ...
                `}>
                    <div className="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden max-h-[80vh] overflow-y-auto">
                        {/* Mobile Search */}
                        <div className="p-3 border-b border-slate-100">
                            <div className="flex items-center bg-slate-50 rounded-xl px-3 py-2">
                                <Search size={18} className="text-slate-400" />
                        ...
                        {/* Mobile Navigation Links with Nested Dropdowns */}
                        <div className="py-2">
                            {navLinks.map((link) => {
                                const Icon = link.icon;
                                const active = isActive(link.href);
                                ...
                                return (
                                    <div key={link.href}>
                                        <div
                                            ...
                                            `}
                                            onClick={() => hasDropdown && setMobileDropdownOpen(!mobileDropdownOpen)}
                                        >
                                            <Link
                                                href={link.href}
                                                onClick={(e) => {
                                                    if (hasDropdown) e.stopPropagation();
                                                    setIsMenuOpen(false);
                                                }}
                                        ...
                                        {hasDropdown && mobileDropdownOpen && (
                                            <div className="bg-slate-50/50 ml-8 my-1 rounded-xl overflow-hidden">
                                                {dropdownItems[link.label as keyof typeof dropdownItems]?.map((item) => {
                                                    const ItemIcon = item.icon;
                                                    return (
                                                        <Link
                                                            ...
                                                            className="flex items-center space-x-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-100 transition-colors"
                                                        >
