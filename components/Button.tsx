...
export default function Button({
    ...
    const handleClick = async (e: any) => {
        if (loading || disabled) return; // prevent double clicks
        try {
            setLoading(true);
            await onClick?.(e); // wait if onClick is async
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <button
            ...
            `}
        >
            {loading && <span className="animate-spin">⏳</span>} {/* optional loading spinner */}
            {icon && <span className="flex items-center">{icon}</span>}
            <span>{label}</span>
        </button>
    );
