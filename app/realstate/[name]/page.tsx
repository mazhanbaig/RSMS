...
// Enhanced Stats Card Component with Theme
const StatsCard = ({ stat, onClick, index }: any) => {
    return (
        <motion.div
            initial={{ opacity: 0, y: 20 }}
                    ...
                    <div className="flex gap-2">
                        <Tooltip title="Refresh Dashboard">
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                onClick={handleRefresh}
                                className="p-2 bg-white rounded-lg shadow-md shadow-slate-200/50 hover:shadow-lg transition-all"
                                disabled={refreshing}
                            >
                        ...
                        <div className="relative">
                            <motion.button
                                ...
                                whileTap={{ scale: 0.95 }}
                                onClick={() => setShowNotifications(!showNotifications)}
                                className="p-2 bg-white rounded-lg shadow-md shadow-slate-200/50 hover:shadow-lg transition-all relative"
                            >
                                            ...
                                            {notifications.length === 0 ? (
                                                ...
                                                notifications.map(notification => (
                                                    <motion.div
                                                        key={notification.id}
                                                        whileHover={{ x: 4 }}
                                                        className={`p-3 hover:bg-slate-50 cursor-pointer transition-all border-b border-slate-50 ${!notification.read ? 'bg-indigo-50/30' : ''}`}
                                                        onClick={() => markNotificationRead(notification.id)}
                                                    >
                ...
                {selectedStat && (
                    ...
                    >
                        <div className="text-center p-6 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl">
                            ...
                            <div className="mt-4">
                                <Badge
                                    count={selectedStat.trend}
                                    className={selectedStat.trendUp ? 'bg-emerald-100 text-emerald-600' : 'bg-rose-100 text-rose-600'}
                                />
                            </div>
                        </div>
